...
#     through gpg.program
#
# This script bypasses all three by reading the passphrase via read -s
# (no echo, no history, no pinentry) and feeding it to a temp wrapper
# that gpg consumes via --passphrase-file. The wrapper + file live in a
# mktemp dir with mode 600 and are wiped on any exit (including Ctrl-C).
#
# Usage:
...
# Read passphrase silently. No pinentry, no copy-paste artifact, no
# leakage via history or process listing.
printf 'GPG passphrase for release key: '
stty -echo
